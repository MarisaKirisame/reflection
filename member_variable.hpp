...
constexpr static bool has_member_variable( \
	...
	< \
		...
		decltype( misc::construct< SELF * >( )->NAME ) \
	>::type * ) \
	{ return true; } \
	template< typename T, typename SELF > \
	...
	typename enable_if_valid< decltype( misc::construct< SELF * >( )->NAME ) >::type \
...
template< typename TYPE, typename NAME >
struct member_variable_type { typedef decltype( misc::construct< TYPE * >( )->template get_member_variable_return_type< NAME, TYPE >( ) ) type; };
template< typename TYPE, typename NAME >
struct member_variable
{
	decltype( misc::construct< TYPE * >( )->template get_member_variable< NAME, TYPE >( ) )
