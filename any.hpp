...
{ \
    ...
    NAME( const T & t ) : \
        ...
        copy_data( []( any_internal * ai ) \
            { \
                typename std::remove_reference< T >::type \
                    t( * static_cast< typename std::remove_reference< T >::type * >( ai->data ) ); \
                return new any_internal_implement< typename std::remove_reference< T >::type >( t ); \
            } ) \
    { \
        ...
        copy_data( []( any_internal * ai ) \
            { \
                typename std::remove_reference< T >::type t( \
                            * static_cast< typename std::remove_reference< T >::type * >( ai->data ) ); \
                return new any_internal_implement< typename std::remove_reference< T >::type >( t ); \
            } ) \
    { \
        if ( any_typename.empty( ) ) \
    ...
    NAME( const NAME & n ) : \
        any_typename( n.any_typename ), any_data( n.copy_data( n.any_data ) ), copy_data( n.copy_data ) { } \
    NAME( NAME && n ) : \
        any_typename( std::move( n.any_typename ) ), \
        any_data( n.copy_data( n.any_data ) ), \
        copy_data( std::move( n.copy_data ) ) { } \
    NAME & operator =( const NAME & n ) \
    { \
        any_typename = n.any_typename; \
        any_data = n.copy_data( n.any_data ); \
        copy_data = n.copy_data; \
        return * this; \
    } \
    ...
    {	\
        ...
        {	\
            ...
            < \
                std::is_same \
                < \
                    typename ::member_function_return_type< T, TAG, ARG ... >::type, \
                    void \
                >::value && \
            ...
            >::type t, \
            const K & k, \
            const ARG & ... arg ) { k( ::call_member_function< T, TAG, ARG ... >( )( * t, arg ... ) ); } \
            template< typename T, typename TAG, typename K, typename ... ARG > \
        ...
        call_member_function_delegate( NAME * that, const KK & k, const AARG & ... a ) : \
            that( that ), k( k ), data( std::addressof( a ) ... ) { } \
        template< int i, typename T, typename ... ARG > \
        typename std::enable_if< i == std::tuple_size< decltype( data ) >::value >::type \
        func( const ARG & ... a ) const \
        { \
            inner::template function \
            < \
                T, \
                TTAG, \
                KK, \
                AARG ... \
            >( static_cast< T * >( that->any_data->data ), k, a ... ); \
        } \
        template< int i, typename T, typename ... ARG > \
        typename std::enable_if< i != std::tuple_size< decltype( data ) >::value >::type \
    ...
    { \
        ...
        { \
            ...
            static void function( \
                ...
                < \
                    std::is_same \
                    < \
                        typename ::static_function_return_type< T, TAG, ARG ... >::type, \
                        void \
                    >::value && \
                    ::has_member_function< T, TAG, ARG ... >::value,\
                    const K &  \
        ...
        void operator( )( const tag< T > & ) const \
        { func< 0, T >( ); } \
        call_static_function_delegate( const KK & k, const AARG & ... a ) : \
            k( k ), data( std::addressof( a ) ... ) { } \
    }; \
    template< typename TAG, typename ... ARG > \
    ...
    { \
        ...
        { k( * static_cast< const TAG * >( that->any_data->data ) ); } \
        template< typename ... > \
        void operator ( )( ... ) const { k( no_existence( ) ); } \
    }; \
    template< typename K > \
    void type_restore( const K & k ) const \
    { string_to_tag( any_typename, type_restore_helper< K >( this, k ) ); } \
};
#endif // ANY_HPP
